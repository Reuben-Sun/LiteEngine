...
namespace ToolEngine
{
	...
	void RenderContext::tick(RenderScene& scene)
	{
		...
		m_command_buffer->setViewport(frame_index, 0, width, 0, height, 0.0f, 1.0f, 0, 1);
		m_command_buffer->setScissor(frame_index, 0, width, 0, height, 0, 1);
		drawUI();
	...
	void RenderContext::setImGuiStyle()
	{
	...
	void RenderContext::drawUI()
	{
		...
		ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), m_command_buffer->getHandle(getFrameIndex()));
	}
	void RenderContext::drawMainMenuBar()
	{
	...
	void RenderContext::drawHierarchy()
	{
	...
	void RenderContext::drawBrowser()
	{
		ImGui::Begin("Browser");
	...
	void RenderContext::drawDetail()
	{
		ImGui::Begin("Detail");
		ImGui::End();
	}
	std::string RenderContext::selectIcon(const std::string& file_name)
	{
